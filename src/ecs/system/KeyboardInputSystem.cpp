//
// Created by antho on 2026-04-01.
//
...
void KeyboardInputSystem::update(const std::vector<std::unique_ptr<Entity>>& entities, const SDL_Event event, float dt) {
        for (auto& e : entities) {
                ...
                if (keyboardInputs.isHoldingAttack) {
                    if (!hasFired.fired) {
                        ...
                        if (projectileLimit.currentProjectiles < projectileLimit.maxProjectiles) {
                            ...
                            if (!isGrounded.grounded) {
                                projectileStats.spawnPoint.y = transform.position.y + (8 * AspectRatioUtil::verticalAspectMult());
                            }
                            else {
                                projectileStats.spawnPoint.y = transform.position.y + (20 * AspectRatioUtil::verticalAspectMult());
                            }
